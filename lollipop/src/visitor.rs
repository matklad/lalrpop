...
use parse_tree::{Node, Symbol};
use ast::AstNode;
use ast::search::traversal;
...
impl<'f, C, V> VisitorBuilder<'f, C, V> {
    ...
    pub fn visit_nodes<'n, F>(self, nodes: &'n [Symbol], f: F) -> VisitorBuilder<'f, C, TyVisitor<'n, V, F>>
        where V: Visit<'f>,
          ...
          F: FnMut(Node<'f>, &mut V::Context),
{
    ...
    fn visit(&mut self, node: Node<'f>, ctx: &mut Self::Context) {
        self.visitor.visit(node, ctx);
        if self.nodes.contains(&node.symbol()) {
            let f = &mut self.f;
            f(node, ctx)
        }
    }
